...
function Historie() {
  ...
  return (
    <div className="d-flex" style={{ minHeight: '100vh' }}>
      {/* Sidebar */}
      <div className="sidebar text-light p-4 pt-5">
        {/* Sidebar content */}
      </div>

      {/* Main content */}
      <div className="container-fluid d-flex flex-column align-items-center bg-light" style={{ marginTop: '50px', paddingBottom: '50px', overflowY: 'auto', flexGrow: 1, marginLeft: '100px' }}>
        <h1 className="text-center mt-5 mb-4">Record</h1>
        <div className="list-group w-100 mb-5" style={{ maxWidth: '800px', height: 'calc(100vh - 150px)', overflowY: 'auto' }}>
          {foodItems.map((item, index) => (
            <div key={index} className="list-group-item border rounded d-flex flex-column flex-md-row justify-content-between align-items-center mt-3 p-3 bg-secondary text-light">
              <div className="flex-grow-1">
                <div>{item.name} - {item.quantity} - {item.description}</div>
